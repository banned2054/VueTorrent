...
export default class QBitProvider implements IProvider {
    ...
    async getDirectoryContent(dirPath: string, mode?: DirectoryContentMode): Promise<string[] | null> {
        return this.post('/app/getDirectoryContent', {
            dirPath,
            mode
        }, {validateStatus: code => code < 500}).then(res => (res.status === 200 ? res.data : null))
    }
    ...
    async getTorrents(payload?: GetTorrentPayload): Promise<QbitTorrent[]> {
        return this.axios.post('/torrents/info', payload).then(r => r.data)
    }

    async getTorrentTrackers(hash: string): Promise<Tracker[]> {
        return this.axios
            .post('/torrents/trackers', {hash})
            .then(r => r.data)
    }
    ...
    async getTorrentFiles(hash: string, indexes?: number[]): Promise<TorrentFile[]> {
        return this.axios
            .post('/torrents/files', {hash, indexes: indexes?.join('|')})
            .then(res => res.data)
            .then(
                (files: TorrentFile[]) =>
                    files.some(file => file.index === undefined)
                        ...
                        : files
                /**
                 * We manually add indexes to the response if they are missing to provide compatibility with older versions of qBittorrent (< 4.4.0)
